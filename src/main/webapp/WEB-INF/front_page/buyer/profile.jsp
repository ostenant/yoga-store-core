...
<script type="text/javascript">
	$(function(){
		...
		$("#province").change(function(){
			...
			$("#town").empty();
			$("#town").append("<option value='' selected='selected'>县/区</option>");
...
<div class="w loc">
	<div class="h-title">
		<div class="h-logo"><a href="http://localhost:8080"><img src="/res/img/pic/logo-1.png"  style="width: 180px;height: 30px;"/></a></div>
	    <div class="h-search">
	      	<input type="text" />
	...
	<div class="r wr profile">
		...
		<div class="confirm">
			...
			<div class="ofc">
				...
				<form id="jvForm" action="/buyer/save/profile.shtml" method="post">
					...
					<li>
						...
						<span class="bg_text"><input type="text" readonly="readonly" name="username" value="${sessionScope.BUYER_ID.username}" maxLength="32"></span>
					</li>
					<li>
						<label for="username">邮　 　 箱：</label>
						<span class="bg_text"><input type="text" name="email" value="${sessionScope.BUYER_ID.email}" maxLength="32"></span>
						<span class="pos" id="e_span" style="display: none;"><span class="tip warningTip">邮箱格式不正确。</span></span>
					</li>
					<li>
						<label for="realName">真 实 姓 名：</label>
						<span class="bg_text"><input type="text" id="realName" name="realName" maxLength="32" value="${sessionScope.BUYER_ID.realName}"/></span>
						<span class="pos" id="r_span" style="display: none;"><span class="tip okTip">&nbsp;</span></span>
					</li>
					<li>
						<label for="gender">性　 　 别：</label>
						<span class="word">
							<input type="radio" name="gender" value="SECRET" <c:if test="${sessionScope.BUYER_ID.gender == 'SECRET'}">checked="checked"</c:if>/>保密　
							<input type="radio" name="gender" value="MAN" <c:if test="${sessionScope.BUYER_ID.gender == 'MAN'}">checked="checked"</c:if>/>男　
							<input type="radio" name="gender" value="WOMAN" <c:if test="${sessionScope.BUYER_ID.gender == 'WOMAN'}">checked="checked"</c:if> />女
						</span>
					</li>
					<li>
						...
						<span class="word">
							<select name="province"  id="province" onchange="changeProvince(this.value)">
								...
								<c:forEach items="${provinces}" var="province">
									<option value="${province.code }" <c:if test="${sessionScope.BUYER_ID.province == province.code}"> selected="selected"</c:if>>${province.name}</option>
								</c:forEach>
							</select>
							<select name="city" id="city">
								<option value="" selected>城市</option>
								<c:forEach items="${cities}" var="city">
									<option value="${city.code }" <c:if test="${sessionScope.BUYER_ID.city == city.code}"> selected="selected"</c:if>>${city.name}</option>
								</c:forEach>
							</select>
							<select name="town" id="town">
								<option value="" selected>县/区</option>
								<c:forEach items="${towns}" var="town">
									<option value="${town.code }" <c:if test="${sessionScope.BUYER_ID.town == town.code}"> selected="selected"</c:if>>${town.name}</option>
								</c:forEach>
							</select>
						</span>
					</li>
					<li><label for="address">详 细 地 址：</label>
						<span class="bg_text"><input type="text" id="address" name="addr" maxLength="32" value="${sessionScope.BUYER_ID.addr}"/></span>
						<span class="pos" id="a_span" style="display: none;"><span class="tip warningTip">详细地址不能超过25位。</span></span>
					</li>
